...
class NetscapeBookmarkParser implements LoggerAwareInterface
{
     ...
     *
     * @param bool   $keepNestedTags Tag bookmarks with parent folder names
     * @param array  $defaultTags    Tag all bookmarks with these values
     ...
     *
     * @return array An associative array containing parsed bookmarks
     */
    ...
    public function parseString($bookmarkString) {
        ...
        foreach ($lines as $line_no => $line) {
            ...
            if (preg_match('/^<h\d.*>(.*)<\/h\d>/i', $line, $m1)) {
                // a header is matched:
                // - bookmarks may be grouped in a (sub-)folder
